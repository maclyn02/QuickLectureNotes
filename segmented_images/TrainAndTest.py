...
def main():
    ...
    try:
        npaClassifications = np.loadtxt("D:\Project\\segmented_images\\classifications.txt", np.float32)     
    except:
    ...
    try:
        npaFlattenedImages = np.loadtxt("D:\Project\\segmented_images\\flattened_images.txt", np.float32)         
    except:
    ...
    imgTestingNumbers = cv2.imread(sys.argv[1])       
    ...
    rt = open(sys.argv[2],"a")
    strFinalString = spell(strFinalString)
    rt.write(strFinalString + "     ")
    rt.close()
    cv2.waitKey(0)
